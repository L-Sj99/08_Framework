...
public class BoardServiceImpl implements BoardService{
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		//    조회될 목록 페이지, 출력할 페이지네이션의 값을 계산할 Pagination 객체 생성하기 
		 ...
		 * - 사용법 : Mapper의 메서드 호출 시 2번째 이후 매개변수로 전달 (1번은 SQL에 전달할 파라미터가 기본값)
		 * */
	...
	public Board selectDetail(Map<String, Integer> map) {
		 ...
		 * (service -> mapper -> DB, DB -> mapper -> service = 왔다 갔다 해야함, 필요한 SELECT 만큼 전체 반복)
		 * 2. MyBatis에서 제공하는 <resultMap>, <collection> 이용하기(자바 코드가 깔끔해보임)
	...
	public Map<String, Object> boardLike(int boardNo, int memberNo) {
		// 1. 좋아요 누른 적 있는지 검사
		int result = mapper.checkBoardLike(boardNo, memberNo); // result == 1 : 누른 적 있음, result == 0 : 누른 적 없음
		...
		// 3. INERT, DELETE 성공 시 해당 게시글의 좋아요 개수 조회
		int count = 0;
		if(result2 > 0) {
	...
	/* 검색 목록 조회 */
	@Override
	public Map<String, Object> selectSearhList(int boardCode, int cp, Map<String, Object> paramMap) {
		// 1. 지정된 게시판에서 검색 조건이 일치하는 게시글이 몇개나 되는지 조회

 ...
 * - 클라이언트 -서버 전이중 통신 담다(중요 객체)
 *  synchronizedSet : 동기화 된 set(충돌 방지, 속도 조금 느림)
 */
	...
	@Override
		...
		// 연결된 클라이언트의 session을 Map에 저장 -> 연결된 클라이언트를 목록화
	...
	// 클라이언트로부터 SockJS.send() 구문을 이용해 텍스트 메시지가 전달된 경우
	@Override
		...
		// 채팅을 보낸 회원의 회원 번호 얻어오기 -> 로그인한 회원번호(session) -> WebSocketSession에 담겨있음(인터섹터 참고)
		...
		// 2) INSERT 서비스 호출(msg값 : chattingRoomNo, messageContent, senderNo, targetNo)
		...
		for(WebSocketSession wss : sessions) {
			// 채팅방에 입장한 사람들(보낸 사람, 받는 사람)에게만 메시지(msg) 전달

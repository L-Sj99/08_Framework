...
public class ChattingServiceImpl implements ChattingService{
	...
	public int chattingEnter(int targetNo, int memberNo) {
		...
		if(chattingNo == 0) {
			// Map 사용 이유 : INSERT 시 새로 생성되는 CHATTING_ROOM_NO 값을 Map에 담아서 얻어오기 위해(useGeneratedKeys=true), <selectKey>
	...
	public List<Message> selectMessage(int chattingNo, int memberNo) {
		...
		if(messageList.isEmpty() == false) {
			...
			// 특정 채팅방의 글 중 내가 보내지 않은 글을 읽음 처리 -> 채팅방 들어가면 모두 읽음 처리하고 메시지 보이기

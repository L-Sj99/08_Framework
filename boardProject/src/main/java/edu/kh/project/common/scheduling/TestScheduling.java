 ...
 * 2. 스케쥴러 코드를 작성할 별도 클래스를 생성한 후 Bean으로 등록 -> @Component 어노테이션 작성
...
public class TestScheduling {
	/* @Scheduled() 매개 변수
	 * 1) fixedDelay : 이전 작업이 끝난 후 다음 작업이 시작 할 때 까지의 시간을 지정
	 * 2) fixedRate  : 이전 작업이 시작한 후 다음 작업이 시작 할 때 까지의 시간을 지정
	 * 3) cron
	 * - UNIX 계열 잡 스케쥴러 표현식

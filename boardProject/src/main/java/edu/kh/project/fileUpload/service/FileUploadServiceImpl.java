...
// @Transactional : Unchecked Excpetion 발생 시 Rollback 수행
// @Transactional(rollbackFor = Exception.class) : Exception 이하 예외 발생 시 Rollback 수행 == Checked, Unchecked 가리지 않고 예외 발생 시 Rollback
@Transactional(rollbackFor = Exception.class)
public class FileUploadServiceImpl implements FileUploadService{
	private final FileUploadMapper mapper;

	@Value("${my.test.web-path}")
	private String testWebPath; // 인터넷 요청 주소
	
	@Value("${my.test.folder-path}")
	private String testFolderPath; // 파일 저장 폴더 경로
	...
	public String test1(MultipartFile uploadFile) throws IllegalStateException, IOException{
		...
		// 1) 업로드 된 파일이 있는지 검사
		if(uploadFile.isEmpty()) { // 업로드 안한 경우
			return null;
		}
		// 2) 지정된 경로에 파일 저장
		...
		// 업로드 되어 메모리 또는 임시 저장된 폴더 있는 파일을 지정된 경로(path)로 전달하는(옮기는) 코드
	...
	public String test2(MultipartFile uploadFile, String fileName) throws IllegalStateException, IOException {
		...
		// 1) 업로드된 파일이 있는지 검사
		// if(uploadFile.getSize() <= 0) 밑에 if 문이랑 같음
		// if(uploadFile. getOriginalFilename().equals("")) 밑에 if문이랑 같음
		 if(uploadFile.isEmpty()) return null; // 업로드 된 파일이 없으면 null 반환
		 ...
		 // 4) DB에 파일 정보부터 INSERT
			FileDto file = FileDto.builder()
					.fileOriginalName(originalName) // 원본 이름
					.fileRename(rename) // 변경된 이름
	...
	public String test3(MultipartFile uploadFile) {
		...
		try {
			 ...
			 * 3) try-catch 작성 시 메서드 내부에서 예외가 처리되어 메서드 종료 시 예외가 던져지지 않아
			 * @Transaction이 rollback을 수행할 수 없게 됨
			 * [추천되는 해결 방법]
			 * * try-catch를 작성해서 Checked Exception을 처리
			 * 	- 호출하는 메서드에 throws 구문 작성 X
			 * * Uncheckec Exception 형태의 사용자 정의 예외 강제 발생
			 * 	- @Transactional 어노테이션에 rollbackFor 속성 작성안해도 rollback 처리 가능
			 */
			
			// 예외 강제 발생 : Unchesked Exception은 컴파일러가 자동으로 throws 구문을 작성해줘서 예외 발생 시 호출부로 던져지게 됨

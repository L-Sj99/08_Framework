...
const selectCommentList = () => {
  ...
  .then(response => {
    // response.ok : HTTP 응답 상태 코드가 200번대(성공)이면 true 아니면 false
    if(response.ok) return response.text();
    throw new Error("댓글 목록 조회를 실패하였습니다.");
  })
  .then(html => {
    ...
    /* [주의 사항]
     * innerHTML로 새로 만들어진 요소에는 이벤트 리스너가 추가되어 있지 않기 때문에 답글, 수정, 삭제 등이 동작하지 않음
     */
...
const insertComment = (parentCommentNo) => {
  ...
  .then(commentNo => {
    ...
    // 알림 클릭 시 이동하는 url에 ?cm=댓글번호 추가 -> 알림 클릭 시 작성된 댓글 또는 답글 위치로 바로 이동

    // 댓글을 작성한 경우 : {닉네임} 님이 {게시글 제목} 게시글에 댓글을 작성했습니다
    if(parentCommentNo === undefined) {
      const content = `<strong>${memberNickname}</strong>님이 <strong>${boardDetail.boardTitle}</strong> 게시글에 댓글을 남겼습니다.`;
      
      /// type, url, pkNo, content
      sendNotification(
        "insertComment", `${location.pathname}?cn=${commentNo}`, boardDetail.boardNo, content
      );
    } else { // 답글(대댓글)을 작성한 경우 : {닉네임}님이 답글을 작성했습니다
      const content = `<strong>${memberNickname}</strong>님이 답글을 남겼습니다.`;

      /// type, url, pkNo, content
      sendNotification(
        "insertChildComment", `${location.pathname}?cn=${commentNo}`, parentCommentNo, content
      );
    }
  })
  .catch(err => console.error(err));
...
const showUpdateComment = (btn) => {
  ...
  cancelBtn.addEventListener("click", () => {
    ...
    // 취소 안함 -> 수정 계속 진행
    if(confirm("취소를 하시겠습니까?") === false) return;

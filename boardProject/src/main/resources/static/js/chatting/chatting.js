/* Websocket : 클라이언트 - 서버 간 전이중 통신
 * ex) 전화 - 양방향
 * - 실시간
 * - 클라이언트1 <-> 서버 <-> 클라이언트2
 * - ws:// 프로토콜 요청 (http 아님) : HTTP에서 사용하는 request, session을 사용할 수 없음.
 * -> SessionHandhakeInterceptor가 HTTP의 Session을 가로채서 Websocket에서 사용 가능한 형태로 변환.
 * - 어떤 요청이 websocket 요청인지 지정
 */
...
if(notificationLoginCheck) { // common.html에 선언된 전역변수
  // 서버로 ws://chattingSock 요청 -> 요청을 처리하는 WebSockHandler와 연결 -> WebSockHandler에 연결된 회원의 정보를 모아둠
...
if(chattingSock != undefined){
  chattingSock.addEventListener("message", e => {
    ...
    if(selectChattingNo == msg.chattingRoomNo){
      ...
      ul.append(li)
      ul.scrTop = ul.scrollHeight; // 스크롤 제일 밑으로
    }
    selectRoomList();
  })
}
...
const chattingEnter = (e) => {
  // e.eurrentTarget : 이벤트 리스너가 설정된 요소
  const targetNo = e.currentTarget.dataset.id; // data-id 값을 얻어와 저장(참여자 회원 번호)
  ...
  .then(response => {
    ...
    throw new Error("채팅방 입장에 실패하였습니다.");
  })
  .then(chattingNo => { // chattingNo : 입장한 채팅방 번호
    ...
    // 입장 하려던 채팅방이 이미 채팅방 목록에 존재하는 경우
    // 1) 채팅방 목록 li 태그들 얻어오기
    const itemList = document.querySelectorAll(".chatting-item");

      // 2) li 태그의 "chat-no" 값과 입장하려는 방 번호가 같은 경우 == 입장 하려는 방이 채팅방 목록에 존재하는 경우
      for(let item of itemList) {
        if(item.getAttribute("chat-no") == chattingNo) {
          item.focus();
          item.click(); // 클릭 -> selectChattingFn() 호출된
...
const selectChattingFn = () => {
  ...
  .then(messageList => {
    ...
    for (let msg of messageList) {
      ...
      ul.append(li);
      ul.scrollTop = ul.scrHeight; // 스크롤 제일 밑으로
    }
  })
  .catch(err => console.log(err));
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector("#inputChatting").addEventListener("keyup", e => {
    ...
    // 입력한 키가 Enter인 경우
    if(e.key == "Enter"){
      if(!e.shiftKey){ /// shift가 눌러지지 않은 경우 == shift + enter 입력 시 제출 X
        sendMessage();
      }
    }
  })

 ...
 *  (서버 -> 클라이언트 데이터 전달 하라는 요청을 또 AJAX를 이용해 비동기 요청)
 */

/* SSE 연결하는 함수 
 * 연결을 요청한 클라이언트가 서버로부터 데이터가 전달 될 때 까지 대기(비동기)
 */
const connectSse = () => {
...
/* 알림 메시지 전송 함수
 * - 알림 받을 특정 클라이언트 id 필요(memberNo 또는 memberNo 를 알아낼 수 있는 값)
 * 
 * [동작 원리]
 * 1) AJAX를 이용해 SseController에 요청
 * 2) 연결된 클라이언트 대기 명단(emitters)에서 클라이언트 id가 일치하는 회원을 찾아 메시지 전달하는 send() 메서드를 수행
 * 3) 서버로 부터 메시지를 전달 받은 클라이언트의 eventSource.addEventListener()가 수행됨
 */

...
/* 필수 입력 항목의 유효성 검사 여부 체크를 위한 객체(체크리스트)*/
const checkObj = {
  "memberEmail" : false,
  "memberPw" : false,
...
// 2) 이메일 메시지를 미리 작성
const emailMessageObj = {}; // 빈객체
emailMessageObj.normal = "인증 받을 수 있는 이메일을 입력해주세요.";
emailMessageObj.invaild = "알맞은 이메일 형식으로 작성해주세요.";
emailMessageObj.duplication = "이미 사용 중인 이메일입니다.";
emailMessageObj.check = "사용 가능한 이메일 입니다.";

// 3) 이메일이 입력될 때 마다 유효성 검사 수행
memberEmail.addEventListener("input", e => {
  ...
  if(inputEmail.length === 0) {
    emailMessage.innerText = emailMessageObj.normal; // emailMessage를 normal 상태 메시지로 변경
    emailMessage.classList.remove("confirm", "error"); // #emailMessage의 색상 관련 클래스를 모두 제거
    checkObj.memberEmail = false; // checkObj에서 memberEmail을 false로 변경
    memberEmail.value = ""; // 잘못 입력된 값(띄어쓰기) 제거
    return;
  }
  // 6) 이메일 형식이 맞는지 검사(정규 표현식을 이용한 검사)
  // 이메일 형식 정규 표현식 객체
  ...
  if(regEx.test(inputEmail === false)){
    ...
    emailMessage.classList.add("error"); // 사용 불가 할 때 글씨 추가
    emailMessage.classList.remove("confirm"); // 사용 가능할 때 글씨 제거
  ...
  .then(count => {
    ...
    if(count == 1) { // 중복인 경우
      emailMessage.innerText = emailMessageObj.duplication; // 중복인 경우 메시지
    ...
    // 중복이 아닌 경우
    emailMessage.innerText = emailMessageObj.check; // 중복이 아닌 경우 메시지
...
nickMessageObj.normal = "한글, 영어, 숫자로만 3~10 글자를 입력해주세요.";
nickMessageObj.invaild = "유효하지 않은 닉네임 형식입니다.";
nickMessageObj.duplication = "이미 사용 중인 닉네임 입니다.";
...
// ----------------------------------------------------------------------------------------------
/* 이메일 인증 */
// 인증 번호를 작성된 이메일로 발송하기
const sendAuthKeyBtn = document.querySelector("#sendAuthKeyBtn"); // 인증번호 받기 버튼
const authKeyMessage = document.querySelector("#authKeyMessage"); // 인증 관련 메시지 출력 span 태그

// 초기 셋팅된 시간
const initTime = "03:00"; // 인증 초기 시간 지정
const initMin = 2; // 초기값 3분에 1초 감소된 분
const initSec = 59; // 초기값 3분에 1초 감소된 후 초 

// 실제 줄어든 시간(분/초)를 저장할 변수
let min = initMin;
let sec = initSec;
let authTimer; // 타이머 역할의 setInterval을 저장할 변수 -> 타이머를 멈추는 clearInterval 수행을 위해 필요
...
sendAuthKeyBtn.addEventListener("click", () =>{
  ...
  // 2) 비동기(멀티태스킹)로 서버에서 작성된 이메일로 인증 코드 발송(ajax)
  ...
  // 3) 이메일 발송 메시지 출력 + 3분 타이머 출력
  alert("인증번호가 발송되었습니다.");
  authKeyMessage.innerText = initTime; // 화면에 03:00 문자열 출력
  authKeyMessage.classList.remove("confirm", "error"); // 검정 글씨로 만들어둠
...
/* 전달 받은 숫자가 10미만(한 자리 수)인 경우 앞에 0을 붙여서 반환하는 함수 */
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  // 1) 인증번호 6자리가 입력 되었는지 확인
  if(authKey.value.trim().length < 6) {
    alert("인증 번호가 잘못 입력되었습니다.");
    return;
  }
  // 2) 입력된 이메일과 인증번호를 비동기로 서버에 전달하여 Redis에 저장된 이메일, 인증번호와 일치하는지 확인

  /* Ajax로 여러 데이터를 서버로 전달하고 싶을 때 JSON 형태로 값을 전달해야함 */
  ...
  fetch("/email/checkAuthKey", {
    method : "POST",
    headers : {"Content-Type" : "application/json"},
    // JSON.stringify : 객체 -> JSON 변환(문자 열화)
  ...
  .then(result => {
    ...
    // 3) 일치하지 않은 경우 alert("인증번호가 일치하지 않습니다.")
    if(result == false) {
      alert("인증 번호가 일치하지 않습니다.")
      checkObj.authKey = false;
      return;
    }
    // 4) 일치하는 경우 타이머 멈춤 + "인증되었습니다."를 화면에 출력 + checkObj.authKey = true;
...
signUpForm.addEventListener("submit", e => {
  // for(let key in 객체) -> 객체의 키값을 하나씩 꺼내서 key 변수에 저장
  for(let key in checkObj) { // 유효성 검사 체크리스트 객체에서 하나씩 꺼내 false인 경우가 있는지 검사
    if(checkObj[key] === false) { // 유효하지 않은 경우
      ...
      switch(key) {
        ...
        case "suthKey" : str = "이메일이 인증되지 않습았습니다."; break;
      }
      alert(str); //경고 출력
      document.getElementById(key).focus(); // 유효하지 않는 요소로 focus 이동
      e.preventDefault(); // 기본 이벤트(form) 제출 막기
      return;
    }
  }

    /* 주소 유효성 검사 - 모두 작성 또는 모두 미작성 */  

...
public class ForwardTestContoller {
	// 기존 Servlet
	// - @WebServlet("요청주소") -> 클래스명 위에 작성 == 해당 클래스를 "요청주소"를 매핑해서 처리하는 클래스
	// -> 클래스 별로 요청주소 1개만 처리가 가능
	 ...
	 * 	
	 * @RequestMapping("요청 주소") 뜻 : 요청 주소를 처리할 클래스 / 메서드를 매핑하는 어노테이션
	 * 1) 클래스 위에 작성하는 경우
	 * - 공통 주소를 해당 클래스에 매핑
	 * ex) /todo/select, /todo/insert, /todo/update... -> @RequestMapping("todo") ==  /todo로 시작하는 요청
	 * 
	 * 2) 메서드 위에 작성하는 경우
	 * - "요청 주소"로 요청을 받은 경우 해당 메서드에서 처리
	 * 
	 * @GetMapping("요청 주소") 뜻 : 
	 * @PostMapping("요청 주소") 뜻 : 
	 */
	...
	// 4) ** 매개 변수는 필요한 만큼 마음대로 -> Arguments Resolver 참조
	// 5) 매핑할 요청 주소를 @RequestMapping 등을 이용해 작성
	
	/* Spring Boot Controller에서 특수한 경우를 제외하고 매핑 주소 제일 앞에 "/"를 적지 않음 */
	@RequestMapping("forward")
	public String forwardTest() {
		System.out.println("/forward 매핑 확인");

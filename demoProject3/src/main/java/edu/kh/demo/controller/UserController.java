...
public class UserController {
	...
	/**  사용자 전체조회
	 * @param : 데이터 전달용 객체(request)
	 * @return user/selectAll.html forward
	 */
	@GetMapping("selectAll")
	public String selectAll(Model model) {
		// service : 의존선 주입(DI) 받은 UserServiceImpl Bean(객체)
	 ...
	 * 2) 리다이렉트 수행 되려고 할 때 1)에서 셋팅된 값을 session scope로 잠깐 (Flash)대피
	...
	public String selectUser(@PathVariable("userNo") int userNo, Model model, RedirectAttributes ra) {
	...
	/* @ModelAttribute
	 * - 전달된 파라미터의 key(name 속성) 값이 작성된 DTO의 필드명과 일치하면
	 * 	 DTO 객체의 필드에 자동으로 셋팅하는 어노테이션
	...
	public String updateUser(@PathVariable("userNo") int userNo, @ModelAttribute UserDto user, RedirectAttributes ra) {
	...
	public String deleteUser(@PathVariable("userNo") int userNo, RedirectAttributes ra) {
	 ...
	 * @param ra : redirect 시 request scope로 값 전달 객체
	 * @return
	 */
	@PostMapping("insert")
	public String insertUser(@ModelAttribute UserDto user, RedirectAttributes ra) {
		
		// INSERT 수행 후
		// 성공 시 : {userName} 님이 추가되었습니다. -> /user/selectAll 리다이렉트
		// 실패 시 : 추가 실패(메시지 전달) -> /user/insert 리다이렉트
			int insert = service.insertUser(user);
			...
			if(insert > 0) {
				path = "redirect:/user/selectAll";
				message = user.getUserName() + "님이 추가가 되었습니다.";
			} else {
				path= "redirect:/user/insert/" + user;
				message = "추가를 실패했습니다";
			}
			ra.addFlashAttribute("message", message);
		return path;
